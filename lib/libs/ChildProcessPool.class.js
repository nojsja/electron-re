  ...
  _createClass(ChildProcessPool, [{
    ...
    value: function getForkedFromPool() {
      ...
      if (!this.pidMap.get(id)) {
        ...
        if (this.forked.length < this.maxInstance) {
          inspectStartIndex++;
          forked = fork(this.forkedPath, this.env.NODE_ENV === "development" ? ['--inspect=' + inspectStartIndex] : [], { cwd: this.cwd, env: _extends({}, this.env, { id: id }) });
    ...
    */
    ...
    value: function setMaxInstanceLimit(count) {
      ...
      if (count < this.maxInstance) console.warn('ChildProcesspool: setMaxInstanceLimit - the param count is less than old value ' + this.maxInstance + ' !');
