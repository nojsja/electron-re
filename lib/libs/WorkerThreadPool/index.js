  ...
  (0, _createClass2["default"])(WorkerThreadPool, [{
    ...
    value: function paramsCheck() {
      ...
      if (taskTime !== undefined && (!Number.isInteger(taskTime) || taskTime < WorkerThreadPool.minTaskTime)) {
        throw new Error("WorkerThreadPool: param - taskTimer must be an positive integer that no less than ".concat(WorkerThreadPool.minTaskTime, "ms."));
      }
    }
  }, {
    ...
    value: function fillPoolWithIdleThreads() {
      ...
      var threads = new Array(countToFill).fill(0).map(function () {
        var thread = WorkerThreadPool.generateNewThread(_this3.execContent, _this3.options.type);
    ...
    value: function consumeTask(task) {
      ...
      if (!this.isFull) {
        var thread = WorkerThreadPool.generateNewThread(this.execContent, this.options.type);
    ...
    value: function send(payload) {
      ...
      return new Promise(function (resolve, reject) {
        var task = WorkerThreadPool.generateNewTask(payload, options);

        if (!_this4.isFull) {
          var thread = WorkerThreadPool.generateNewThread(_this4.execContent, _this4.options.type);
    ...
    value: function generateNewThread(execContent, type) {
...
WorkerThreadPool.minTaskTime = 100;
