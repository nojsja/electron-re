...
var LoadBalancer = /*#__PURE__*/(0, _createClass2["default"])(
/**
...
function LoadBalancer(options) {
  ...
  this.memorizedParams = function () {
    ...
    return _ref = {}, (0, _defineProperty2["default"])(_ref, RANDOM, function () {
      ...
      return [_this.params.weightTotal, _this.params.connectionsMap, _this.params];
    }), _ref;
  };

  this.refreshParams = function (pidMap) {
    var infos = Object.values(pidMap);

    for (var _i = 0, _infos = infos; _i < _infos.length; _i++) {
      var info = _infos[_i];
      // this.params.connectionsMap[id] = connections;
      _this.params.cpuOccupancyMap[info.pid] = info.cpu;
      _this.params.memoryOccupancyMap[info.pid] = info.memory;
    }
  };
  ...
  this.calculateWeightIndex = function () {
    _this.params.weightTotal = _this.targets.reduce(function (total, cur) {
      return total + (cur.weight || 0);
    }, 0);

    if (_this.params.weightIndex > _this.params.weightTotal) {
      _this.params.weightIndex = _this.params.weightTotal;
    }
  };

  this.calculateIndex = function () {
    if (_this.params.currentIndex >= _this.targets.length) {
      _this.params.currentIndex = _this.params.currentIndex - 1 >= 0 ? _this.params.currentIndex - 1 : 0;
    }
  };
  ...
  this.clean = function (id) {
    if (id) {
      ...
      delete _this.params.memoryOccupancyMap[id];
    } else {
      _this.params = {
        currentIndex: 0,
        connectionsMap: {},
  ...
  this.del = function (target) {
    ...
    for (var i = 0; i < _this.targets.length; i++) {
      if (_this.targets[i].id === target.id) {
    ...
    if (found) {
      ...
      console.warn("Del Operation: the task ".concat(target.id, " is not found."), _this.targets);
    }
  };
