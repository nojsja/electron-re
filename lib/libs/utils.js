...
exports.loadView = function (_ref) {
  ...
  function internalFunc() {
    ...
    var needPolyfill = compareVersion(process.versions.electron, '14') >= 0;
    global._depends = [];
    global.$require = require;

    global.require = require = function (require) {
      var _require = require;
      var remote = needPolyfill ? _require('@electron/remote') : _require('electron').remote;
      return function (_path) {
        var result;

        var path = _require('path');

        if (_path === 'electron') return _objectSpread(_objectSpread(_objectSpread({}, needPolyfill ? remote.electron : remote.require('electron')), _require('electron')), {}, {
          remote: remote,
          remoteRequire: function (name) {
            if (needPolyfill) return this[name];
