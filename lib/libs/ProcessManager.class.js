  ...
  _createClass(ProcessManager, [{
    ...
    value: function pipe(pinstance) {
      ...
      if (pinstance.stdout) {
        pinstance.stdout.on('data', function (trunk) {
          _this3.processWindow && _this3.processWindow.webContents.send('process:stdout', {
            pid: pinstance.pid,
            data: String.prototype.trim.call(trunk)
          });
        });
      }
    }
    ...
    value: function listen(pids) {
      ...
      pids.forEach(function (pid) {
        if (!_this4.pidList.includes(pid)) {
          _this4.pidList.push(pid);
        }

        _this4.typeMap[pid] = mark;
      });
    }
        ...
        return regeneratorRuntime.wrap(function _callee3$(_context3) {
          while (1) {
            switch (_context3.prev = _context3.next) {
              case 0:
                _this5.processWindow = new BrowserWindow({
                  show: false,
                  width: 600,
                  height: 400,
                ...
                _this5.processWindow.once('ready-to-show', function () {
                  _this5.processWindow.show();

                  _this5.setTimer(2e3);

                  ipcMain.on('process:kill-process', function (event, args) {
                    return _this5.killProcess(args);
                  });
                  ipcMain.on('process:open-devtools', function (event, args) {
                    return _this5.openDevTools(args);
                  });
                });

                _this5.processWindow.loadURL(loadingUrl);
